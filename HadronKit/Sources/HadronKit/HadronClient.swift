...
public enum HadronError: LocalizedError {
...
/// Thin GraphQL client for the Hadron API. Stateless apart from the config and
/// bearer token it is constructed with.
public struct HadronClient {
    public let config: HadronClientConfig
    public let token: String

    public init(config: HadronClientConfig, token: String) {
        self.config = config
        self.token = token
    }
    ...
    public func taskNodes(memoryId: String? = nil, limit: Int = 200) async throws -> [HadronNode] {
    ...
    /// Browse a single memory's nodes (no search query — a deterministic
    /// filtered list, sorted by loc so the tree order is stable).
    public func memoryNodes(memoryId: String, limit: Int = 200) async throws -> [HadronNode] {
        struct Payload: Decodable { let findNodes: FindNodesResult }
        let query = """
        query($filter: NodeFilter, $limit: Int) {
          findNodes(filter: $filter, sort: loc, limit: $limit) {
            hits { node { \(Self.nodeFields) } }
          }
        }
        """
        let variables: [String: Any] = [
            "filter": ["memoryIds": [memoryId]],
            "limit": limit,
        ]
        return try await run(query, Payload.self, variables: variables).findNodes.nodes
    }

    /// Fetch one node's full detail. `ref` accepts a node PK or a
    /// fully-qualified URN; denied and missing both come back nil (the server
    /// deliberately doesn't distinguish them on this surface).
    public func node(ref: String) async throws -> HadronNodeDetail? {
        struct Payload: Decodable { let node: HadronNodeDetail? }
        let query = """
        query($ref: ID!) {
          node(ref: $ref) {
            id loc name description abstract content nodeType tags updatedAt urn
            memory { urn name }
          }
        }
        """
        return try await run(query, Payload.self, variables: ["ref": ref]).node
    }

    public func findNodes(search: String, limit: Int = 50) async throws -> [HadronNode] {
        // The search box is free-form text, but findNodes' keyword mode
        // parses a boolean query grammar — send a literal-safe form so no
        // user input is rejected as malformed syntax.
    ...
    public static func literalSafeQuery(_ raw: String) -> String {
    ...
    /// far beyond what the apps can usefully show.
    ...
    private func run<T: Decodable>(
        ...
        var request = URLRequest(url: config.graphQLURL)
