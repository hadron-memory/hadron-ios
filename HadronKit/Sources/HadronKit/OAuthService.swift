...
@MainActor
public final class OAuthService: NSObject, ASWebAuthenticationPresentationContextProviding {
    ...
    private let config: HadronClientConfig
    ...
    private func ensureClientId(registrationEndpoint: String?) async throws -> String {
        if let cached = keychain.get(.clientId) { return cached }

        let endpoint = registrationEndpoint.flatMap(URL.init(string:))
            ?? config.baseURL.appendingPathComponent("oauth/register")
        ...
        keychain.set(decoded.client_id, for: .clientId)
    ...
    private func buildAuthorizeURL(endpoint: String, clientId: String, pkce: PKCEPair, state: String) -> URL? {
        ...
        components.queryItems = [
            ...
            .init(name: "resource", value: config.resource),
